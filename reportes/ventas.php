...
*/
	...
	if(!$conDB){
    	...
    	$comprobar=mysqli_query($conDB,$caja);
    		...
    		$mipdf -> Cell(0, 3,'Reporte de Ventas', 0, 0, 'C');
    		$mipdf -> Ln();
			$mipdf -> SetFont('Helvetica', 'B', 11);

			$mipdf -> SetWidths(Array(3,3,5,2,2,2,2));
		...
		for($j=0; $j< count($registros); $j++){
			$mipdf -> Row(array(
				utf8_decode($registros[$j]['fecha']),
				utf8_decode($registros[$j]['codigo']),
				utf8_decode($registros[$j]['descripcion']),
				utf8_decode($registros[$j]['cantidadArt']),	
				utf8_decode($registros[$j]['costo']),
				utf8_decode($registros[$j]['precio']),
				utf8_decode($registros[$j]['total'])
			));
		}
	$mipdf -> Output('Reporte-Ventas.pdf', 'I');
	}    
?>