...
*/
	...
	if(!$conDB){
		...
		$caja="SELECT* FROM articulos WHERE linea='Accesorios'";
		...
		$mipdf -> Cell(0, 3,'REPORTE DE ARTICULOS "ACCESORIOS"', 0, 0, "C");
    	$mipdf -> Ln();
		...
		for($j=0; $j< count($registros); $j++){
			$mipdf -> Row(array(
				...
				utf8_decode($registros[$j]['proveedor2']),					utf8_decode($registros[$j]['proveedor3']),
				utf8_decode($registros[$j]['costo']),
				utf8_decode($registros[$j]['precio'])
			));
		}
	$mipdf -> Output('Reporte-Articulos-Accesorios.pdf', 'I');
	}
?>